<div>
    <h2 class="text-lg font-bold mb-2">Form Permintaan Barang (Keranjang)</h2>
    ...
    <form wire:submit.prevent="addToCart" class="flex gap-2 mb-4">
        ...
        <button type="submit" class="px-4 py-2 rounded font-semibold transition-colors bg-blue-600 dark:bg-blue-500 text-white hover:bg-blue-700 dark:hover:bg-blue-400 focus:outline-none focus:ring-2 focus:ring-blue-400">Add</button>
    </form>
    ...
    <div class="mb-4">
        <h3 class="font-semibold mb-2">Daftar Barang Pengajuan</h3>
        @if(count($cart) > 0)
                ...
                @foreach($cart as $index => $c)
                    <div class="border border-gray-300 dark:border-gray-600 rounded p-2 flex items-center justify-between bg-white dark:bg-gray-800 shadow">
                        <div>
                            ...
                            Stok: <span class="text-gray-600 dark:text-gray-300">{{ $c['stock'] }}</span>
                        </div>
                        <button wire:click="removeFromCart({{ $index }})" class="px-2 py-1 rounded font-semibold transition-colors bg-red-600 dark:bg-red-500 text-white hover:bg-red-700 dark:hover:bg-red-400 focus:outline-none focus:ring-2 focus:ring-red-400">Hapus</button>
                    </div>
                @endforeach
            </div>
        @else
            <div class="text-gray-500 dark:text-gray-300">Belum ada barang di daftar.</div>
        @endif
    </div>
    <button wire:click="submitCart" class="px-4 py-2 rounded font-semibold transition-colors bg-green-600 dark:bg-green-500 text-white hover:bg-green-700 dark:hover:bg-green-400 focus:outline-none focus:ring-2 focus:ring-green-400" @if(count($cart) == 0) disabled @endif>Ajukan Permintaan</button>
</div> 